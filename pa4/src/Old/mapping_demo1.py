...
class MappingNode(object):
    ...
    def __init__(self):
        ...
        # Publishers
        self.map_pub = rospy.Publisher('/my_static_map', OccupancyGrid, queue_size=1)
        self.score_pub = rospy.Publisher('/p1_demo1/answer', OccupancyGrid, queue_size=1)
        ...
        #self.pose_sub = rospy.Subscriber('/my_dead_reckoning', PoseStamped, self.updatePose)
        #self.pose_msg = None
        #self.scan_sub = rospy.Subscriber('/robot0/scan', LaserScan, self.updateMap)
        #self.ts.registerCallback(self.updateMap)
    ...
    def publishOccupancyGrid(self):
        ...
        # if there are cells with a value of 50 (unknown) then convert to -1
        halfs = pre_occ_grid == 50
        pre_occ_grid[halfs] = -1
        self.ros_occ_grid.data = np.ravel(pre_occ_grid.reshape(1,-1)).tolist()
        
        # Publish the grid
        self.map_pub.publish(self.ros_occ_grid)
        self.score_pub.publish(self.ros_occ_grid)
