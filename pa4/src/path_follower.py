...
class PathFollowerNode(object):
    ...
    def executeLastTwist(self):
        ...
        thetag = self.current_goal[2]
        
        # Caculate rho to see if we are in tolerance of goal
        rho = math.sqrt((y - yg)**2 + (x - xg)**2 + (theta-thetag)**2)

        # If we are not within tolerance, calculate the control values
        if rho > 0.07: # Grading for PA4 is according to rho > .1
            self.moving = True
            ...
            vel_msg.angular.z = self.ka*a+self.kb*b
